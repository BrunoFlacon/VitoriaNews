// Ported from supabase/functions/get-analytics (Deno) → Node/Express local runtime.
// Aggregates analytics data from multiple tables for the dashboard.
...
export default async function getAnalytics({ body, query, user, supabase }) {
  ...
  console.log(`[get-analytics] userId=${userId}, period=${period}, platform=${platform}`);
  ...
  const getD = (r) => r.status === "fulfilled" ? (r.value.data || []) : [];
  ...
  console.log(`[get-analytics] Posts: ${rawPosts.length}, Contas: ${socialAccounts.length}, Mensagens: ${messages.length}`);
  ...
  for (const ch of msgChannels) {
    ...
    if (externalId && processedExternalIds.has(`${p}_${externalId}`)) continue;
  ...
  const metrics = postMetrics.filter(m => {
    ...
    if (source === "dashboard") return m.post_id && dashboardPostIds.has(m.post_id);
    return true;
  });
  ...
  // ─── Dashboard source: inject messages & posts ─────────────────
  ...
  const scData = gaData.filter(g => g.metric_name === "search_console");
  const scStats = scData.length > 0 ? scData.reduce((acc, g) => {
    const m = g.metadata || {};
    return {
  ...
  const getDateKey = (dateStr, is24h) => {
    const d = new Date(dateStr);
    if (is24h) { d.setMinutes(0, 0, 0); return d.getTime(); }
    d.setHours(0, 0, 0, 0);
    return d.getTime();
  };
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    chartData.push({
      name: is24h
        ...
        : targetDate.toLocaleDateString("pt-BR", { day: "numeric", month: "short" }),
  ...
  return json({
    ...
    engagement: {
      views: tViews, likes: tLikes, comments: tComms, shares: tShares,
      reach: tReach || totalFollowers,
      engagementRate: tViews > 0 ? ((tLikes + tComms + tShares) / tViews * 100).toFixed(2) : "0",
      growth: "0",
    },
    chartData,
    platformBreakdown,
    ...
    bestTimes,
    followerData: accounts.filter(a => {
      if ((a.platform === 'whatsapp' || a.platform === 'telegram') && String(a.id).includes('_agg_')) {
        return !accounts.some(o => o.platform === a.platform && !String(o.id).includes('_agg_'));
      }
      return true;
    }),
    adsStats: metaAdsConfigured ? ads.reduce((acc, ad) => ({
      impressions: acc.impressions + (ad.impressions || 0),
      reach: acc.reach + (ad.reach || 0),
      clicks: acc.clicks + (ad.clicks || 0),
      spend: acc.spend + (ad.amount_spent || 0),
    }), { impressions: 0, reach: 0, clicks: 0, spend: 0 }) : null,
    googleAdsStats: googleAdsConfigured ? googleAds.reduce((acc, g) => ({
      impressions: acc.impressions + (g.impressions || 0),
      clicks: acc.clicks + (g.clicks || 0),
      cost: acc.cost + ((g.cost_micros || 0) / 1000000),
