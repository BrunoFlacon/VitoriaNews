...
import { daysAgoStart, lastNDayKeys, localDayKey, mondayIndex, startOfLocalDay } from "./crm-date-utils";
import type {
  ActivityItem,
...
export async function loadMetrics(db: DB): Promise<MetricsBundle> {
  ...
  const [
    ...
    db.from("whatsapp_conversations").select("id", { count: "exact", head: true }).eq("status", "open"),
    db.from("whatsapp_conversations").select("id", { count: "exact", head: true }).eq("status", "open").gte("created_at", todayStart),
    db.from("whatsapp_conversations").select("id", { count: "exact", head: true }).eq("status", "open").gte("created_at", yesterdayStart).lt("created_at", todayStart),
    ...
    db.from("messages").select("id", { count: "exact", head: true }).eq("sender_type", "agent").gte("created_at", yesterdayStart).lt("created_at", todayStart),
  ]);
...
export async function loadActivity(db: DB, limit = 20): Promise<ActivityItem[]> {
  ...
  for (const m of (msgs.data ?? []) as unknown as Array<{
    ...
    items.push({ id: `msg-${m.id}`, kind: "message", text: `Nova mensagem de ${who}`, at: m.created_at });
  }

  for (const c of (contacts.data ?? []) as Array<{ id: string; name: string | null; phone: string; created_at: string }>) {
    items.push({ id: `contact-${c.id}`, kind: "contact", text: `Novo contato: ${c.name || c.phone}`, at: c.created_at });
  }
  ...
  for (const d of (deals.data ?? []) as unknown as Array<{
    ...
    items.push({
      ...
      text: stage?.name ? `Negócio "${d.title}" em ${stage.name}` : `Negócio "${d.title}" atualizado`,
      at: d.updated_at,
    });
  }
  ...
  for (const b of (broadcasts.data ?? []) as Array<{ id: string; name: string; status: string; total_recipients: number; created_at: string }>) {
    ...
    items.push({ id: `broadcast-${b.id}`, kind: "broadcast", text: `Transmissão "${b.name}" ${label}`, at: b.created_at });
  }
