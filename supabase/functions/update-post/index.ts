...
import { updatePlatformPost } from '../_shared/platforms/post-sync.ts';
...
async function logSync(supabase: any, entry: {
  ...
  try {
    ...
    console.error("[update-post] Error writing post_sync_log:", e);
  }
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { postId, content, mediaUrls, platforms, userId: bodyUserId } = await req.json();
    ...
    if (content === undefined && mediaUrls === undefined) {
      return new Response(JSON.stringify({ success: false, error: "Envie content e/ou mediaUrls para editar" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }
    ...
    if (!rows || rows.length === 0) {
      return new Response(JSON.stringify({
        success: false,
        error: "Nenhuma publicação encontrada para este post (ou foi excluída).",
        results: []
      }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    ...
    for (const row of rows) {
      const result = await updatePlatformPost(supabase, row, { content, mediaUrls });
      results.push(result);
      ...
      await logSync(supabase, {
        ...
        metadata: { unsupported: result.unsupported || false, content }
      });

      if (result.success) {
        await supabase
          .from("published_posts")
          .update({ status: "edited", edited_at: new Date().toISOString(), last_sync_at: new Date().toISOString() })
          .eq("id", row.id);
      }
    }

    // Atualiza o conteúdo do agendamento (fonte do post) quando houver sucesso
    if (results.some((r: any) => r.success) && content !== undefined) {
      try {
        await supabase
        ...
        console.error("[update-post] Error updating scheduled post:", e);
      }
    }
    ...
    console.error("[update-post] Fatal error:", error);
